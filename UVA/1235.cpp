...
int main(int argc, char const *argv[]) {
    ...
    for (int cs = 0; cs < t; ++cs) {
        ...
        for (int i = 0; i < n; ++i) {
            cin >> key[i];
            for (int j = 0; j < i; ++j) {
                K->addEdge(i, j, inner_product(all(key[i]), key[j].begin(), 0, plus<int>(), dist()));
            }
        }
        int init = 10000;
        string start = "0000";
        for (int i = 0; i < n; ++i) {
            init =  min(init, inner_product(all(key[i]), start.begin(), 0, plus<int>(), dist()));
        }
        cout << K->MST() + init << endl;
    }
